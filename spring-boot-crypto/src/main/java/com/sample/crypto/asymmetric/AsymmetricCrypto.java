...
import com.sample.crypto.symmetric.SymmetricAlgorithm;
import com.sample.crypto.utils.CharsetUtil;
import com.sample.crypto.utils.RadixUtil;
import com.sample.crypto.utils.StrUtil;

/**
 * 非对称加密算法<br>
 * 1、签名：使用私钥加密，公钥解密。用于让所有公钥所有者验证私钥所有者的身份并且用来防止私钥所有者发布的内容被篡改，但是不用来保证内容不被他人获得。<br>
 * 2、加密：用公钥加密，私钥解密。用于向公钥所有者发布信息,这个信息可能被他人篡改,但是无法被他人获得。 TODO
 * 针对字节的加密解密处理，目前还没搞的太清楚，公私钥对的处理
 */
public class AsymmetricCrypto implements Crypto {
	 ...
	 * 构造，创建新的私钥公钥对
	 * 
	 * @param algorithm {@link com.sample.crypto.symmetric.SymmetricCrypto}
	 */
	 ...
	 * {@link AsymmetricCrypto#setSignature(Signature)}设置签名对象
	 * 
	 * @param algorithm  算法
	 * @param privateKey 私钥
	 * @param publicKey  公钥
	...
	public AsymmetricCrypto init(String algorithm, byte[] privateKey, byte[] publicKey) {
		final PrivateKey privateKeyObj = (null == privateKey) ? null
				: KeyGenerator.generatePrivateKey(algorithm, privateKey);
		final PublicKey publicKeyObj = (null == publicKey) ? null
				: KeyGenerator.generatePublicKey(algorithm, publicKey);
	...
	public String encrypt(String src, String charset) {
		return RadixUtil.toHex(encrypt(StrUtil.bytes(src, CharsetUtil.charset(charset)), KeyType.PublicKey));
	}
	...
	public String encrypt(String data) {
		return RadixUtil.toHex(encrypt(StrUtil.bytes(data, CharsetUtil.CHARSET_UTF_8), KeyType.PublicKey));
	}
	...
	public String decrypt(String hex) {
		return new String(decrypt(RadixUtil.toBytes(hex), KeyType.PrivateKey));
	}
